...
def analyse_pair(s1, s2):
    ...
    # Filter the data from 2015-01-01 to 2020-12-31
    df = df.loc['2015-01-01':'2020-12-31']

    cash = 100000000  # initial cash
    shares = 0  # number of shares held, positive for long, negative for short
    position = 0  # -1 for short, 1 for long, 0 for no position
    trade_log = []
    shares_per_trade = 200  # 2 hands
    stop_loss = None
    
    for date, row in df.iterrows():
        z_score = row['Z-Score']
        current_value = cash + shares * row[f'{s1}_Close']
        
        # Check if the portfolio value falls below the stop loss value
        if stop_loss is not None and current_value < stop_loss:
            # Close any position and set stop_loss back to None
            if position == 1:
        ...
        if z_score > 1.5 and position <= 0:
            # Close short position if any and open long position
            if position == -1:
                cash += shares * row[f'{s1}_Close']
                shares = 0
            ...
            # Close long position if any and open short position
            if position == 1:
                cash += shares * row[f'{s1}_Close']
                shares = 0
            shares += shares_per_trade
    ...
    final_value = cash + shares * df.iloc[-1][f'{s1}_Close']
    ...
    returns = df[f'{s1}_Close'].pct_change()
    annual_sharpe_ratio = np.sqrt(252) * returns.mean() / returns.std()
    num_trades = len(trade_log)

    roll_max = df[f'{s1}_Close'].cummax()
    daily_drawdown = df[f'{s1}_Close'] / roll_max - 1.0
    ...
    df['PortfolioValue'] = cash + (df[f'{s1}_Close'] * shares)
...
print('\nMetrics:')
metrics_df = metrics_df.applymap('{:,.4f}'.format)  # Display 4 decimal places
print(metrics_df)
